...
run_ora <- function(query_genes, relaxed_path, sig_path, label = "") {
  ...
  if (length(query_genes) < 1) {
    ...
    cat(sprintf("  %-26s no query genes in background -> skip\n", label)); return(invisible(NULL))
  }
  res_obj <- tryCatch(
  ...
  if (is.null(res) || !nrow(res)) {
    write.table(empty_cols(), relaxed_path, sep = "\t", quote = TRUE, row.names = FALSE)
    write.table(empty_cols(), sig_path,     sep = "\t", quote = TRUE, row.names = FALSE)
    cat(sprintf("  %-26s tested=0  (query=%d)\n", label, length(query_genes))); return(invisible(NULL))
  }
  ...
  cat(sprintf("  %-26s tested=%4d  significant(<=0.1)=%4d  (query=%d)\n",
              label, nrow(out), nrow(sig), length(query_genes)))
  invisible(out)
}
...
studies <- list(
  list(expr = "glds270_fc_and_pvals.csv", id = "270", sig = "GLDS-270-ORA.tsv"),
  list(expr = "glds580_fc_and_pvals.csv", id = "580", sig = "GLDS-580-ORA.tsv"),
  list(expr = "glds599_fc_and_pvals.csv", id = "599", sig = "GLDS-599-ORA.tsv")
)
cat(sprintf("\nSingle-study g:Profiler (top-%d |log2FC| genes)\n", ORA_TOPK))
for (s in studies) {
  ...
  #uni <- map_entrez(fc$gene)                                  # full expressed universe — unchanged
