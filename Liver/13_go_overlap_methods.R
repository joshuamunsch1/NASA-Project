# ============================================================
# 13_go_overlap_methods.R  (method benchmarking: convergence & complementarity)
#   inputs (from 11_gsea.R / 11b_ora.R):
#     data/go_bp_terms/<arm>_<method>_gseGO_high_pval.tsv   (GSEA)
#     data/go_bp_terms/<arm>_<method>_ora_high_pval.tsv     (ORA)
#     data/go_bp_terms/<id>_gseGO_high_pval.tsv             (single GSEA)
#     data/go_bp_terms/<id>_ora_high_pval.tsv               (single ORA)
#     data/go_terms_single_datasets/GLDS-<id>-GO.tsv  / -ORA.tsv            (totals)
#
#   outputs: data/go_bp_terms/method_overlap/
#     (A) between_arms_presence.csv      one stacked table; Method + Paradigm
#                                        columns; all-arm presence + n_arms +
#                                        is_core (sig in every present arm)
#         between_arms_jaccard.csv       long arm_a x arm_b jaccard, Method +
#                                        Paradigm columns
#     (B) merged_vs_single_summary.csv   every (method, paradigm) x arm x study
#                                        summary row (no longer collapsed by
#                                        method -- a method can appear under both
#                                        paradigms)
#         merged_vs_single_details.csv   all per-pair matched terms, stacked
#                                        (Method/Paradigm/Arm/RR_Mission columns)
#     (C) panel_presence_matrix.csv, panel_shared_by_k.csv,
#         panel_cell_summary.csv, convergence_within_paradigm.csv
#     (D) paradigm_complementarity.csv   ALL_ARMS + per-arm scope rows
#         paradigm_terms.csv             shared / gsea_only / ora_only term lists
#                                        stacked with a 'class' column
# ============================================================
...
SIG_CUT <- 0.1
WRITE_PAIR_DETAILS <- TRUE      # stacked per arm x study matched-term file (B)
...
STUDIES <- list(
  list(rr = "RR1_NASA", id = "168_RR1", sig_gsea = "GLDS-168_RR1-GO.tsv", sig_ora = "GLDS-168_RR1-ORA.tsv"),
  list(rr = "RR6",      id = "245",     sig_gsea = "GLDS-245-GO.tsv",     sig_ora = "GLDS-245-ORA.tsv"),
  list(rr = "RR8",      id = "379",     sig_gsea = "GLDS-379-GO.tsv",     sig_ora = "GLDS-379-ORA.tsv"),
  list(rr = "RR9",      id = "242",     sig_gsea = "GLDS-242-GO.tsv",     sig_ora = "GLDS-242-ORA.tsv")
)

# relaxed-file suffix per paradigm
relx <- function(paradigm) if (paradigm == "gsea") "gseGO_high_pval" else "ora_high_pval"

# paradigm is now explicit, so the same method resolves to two different files.
...
# cell key carries paradigm so a method present in both paradigms never collides.
...
# ------------------------------------------------------------
# Load the whole panel once: sig IDs per cell, plus a description lookup.
# ------------------------------------------------------------
cell_sig  <- list()   # "<arm>__<method>__<paradigm>" -> significant GO IDs
cell_meta <- list()
...
# ============================================================
# (B) MERGED vs SINGLE, per (method, paradigm)
#     -> one summary file + one stacked details file
# ============================================================
...
# ============================================================
# (C) PANEL CONVERGENCE  (pool all cells)
# ============================================================
...
# ============================================================
# (D) CROSS-PARADIGM COMPLEMENTARITY  (GSEA-union vs ORA-union)
# ============================================================
